...
func _physics_process(_delta: float) -> void:
	...
	if frame_data.get("interact", false):
		for area in _interactables:
			if is_instance_valid(area) and area.has_method("on_echo_interact"):
